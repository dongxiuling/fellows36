...
class index extends Component {
    ...
    // 箭头函数 改变this指向
    createMenu = (menu) =>{
        let list = menu.map((obj,index)=>{
            if(obj.children){
                return (<SubMenu key={obj.path} title={obj.title}>
                    {/* obj.children []  =》 <Menu.Item >  函数的递归调用*/}
                    {
        ...
        return (
            <div className="nav-box">
                ...
                <Menu mode="inline" theme="dark">
                    {/* <Menu.Item key="01">Option 1</Menu.Item>
                    <Menu.Item key="02">Option 1</Menu.Item>
                    <SubMenu key="3" title="菜单3">
                        <Menu.Item key="03">Option 1</Menu.Item>
                    </SubMenu> */}
                    {this.state.list}
                </Menu>
            </div>
